...
export default class BluetoothDoor{
    ...
    openDoor(){
        let curItem={
            bluetoothName: this.data.bluetoothName,
            productKey: this.data.productKey,
            bluetoothSN: this.data.bluetoothSN
        };
        console.log(curItem)
        this.setData({curItem});
        wx.showLoading({ title: '开门中...', mask: true });
        this.openDoorOnce().then(async ()=>{
            await wx.hideLoading();
            await wx.showToast({ title: '开门完成', duration: 1500, icon: 'success' });
        }).catch(async (err)=>{
            const err_msg=typeof err=='string' ?err:'未知错误';
            await wx.hideLoading();
            //console.log(err.errMsg)
            await wx.showToast({ title: err.message||err_msg, duration: 1500, icon: 'error' });
            console.error(err)
        }).finally(async ()=>{
    ...
    async openDoorOnce() {
        ...
        const device_name = this.data.curItem.bluetoothName;
        const device = await this.findDevice(device_name);
    ...
    async findDevice(deviceName) {
        ...
        if (res_find.errno === 0) {
            console.log('正在搜索蓝牙设备' + deviceName + '...', res_find);
        } else {
        ...
        return new Promise((resolve, reject) => {
            ...
            wx.onBluetoothDeviceFound(async (vo) => {
                ...
                if (device) {
                    console.log('已找到蓝牙设备 ' + deviceName, device)
