...
export default class BluetoothDoor{
      ...
      this.openDoorOnce(list).then(async ()=>{
         ...
         // wx.setStorageSync('curItem', curItem);
          try {
            ...
            console.error('closeAction error',error)
          }
      });
  }
    ...
    isIosApp(){
      ...
      const host=app.host;
      return !!host.bundleIdentifier;
    }

    async stopBluetoothDevicesDiscovery(){
       if(this.isIosApp()){
          console.log('ios APP 无法执行stopBluetoothDevicesDiscovery');
          return {errno:0};//ios APP 默认执行成功
       }else{
    ...
    async closeAction(){
      if(this.is_search){
        let res = await this.stopBluetoothDevicesDiscovery();
        console.log('finally 执行停止搜索',res);
      }
      if(this.is_connect&&this._deviceId){
        let res=await wx.closeBLEConnection({ deviceId: this._deviceId});
        this.is_connect=false;
        console.log('finally 执行关闭连接',res);
      }
     if(this.is_open){
        let  res=await wx.closeBluetoothAdapter();
        console.log('finally 执行关闭蓝牙适配器',res);
    }
  }

    async openDoorOnce(deviceNameList) {
        let res=await wx.closeBluetoothAdapter();
        console.log('预先关闭蓝牙。。',res);
